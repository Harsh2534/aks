...
from info import LINK_MODE, FILE_CAPTION, TUTORIAL, TUTORIAL_2, DATABASE_NAME, DATABASE_URI, DATABASE_URI2, IMDB, IMDB_TEMPLATE, PROTECT_CONTENT, AUTO_DELETE, SPELL_CHECK, AUTO_FILTER, LOG_VR_CHANNEL, SHORTENER_WEBSITE, SHORTENER_API, SHORTENER_WEBSITE2, SHORTENER_API2, TWO_VERIFY_GAP, IS_VERIFY, AUTH_CHANNEL, URL
...
class Database:
    ...
    async def add_chat(self, id, title):
        chat = {'id': id, 'title': title, 'settings': default}
    ...
    async def get_stream_url(self, bot_id):
        bot = await self.botcol.find_one({'id': bot_id})
        if bot and bot.get('stream_url'):
            return bot['stream_url']
        else:
            return URL

    async def update_stream_url(self, bot_id, value):
        user = await self.botcol.find_one({'id':int(bot_id)})
        if user:
            await self.botcol.update_one({'id': int(bot_id)}, {'$set': {'stream_url': value}})
        else:
            await self.botcol.insert_one({'id': int(bot_id), 'stream_url': value})
